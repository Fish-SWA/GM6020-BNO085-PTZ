...
int circle_err(int val_a, int val_b, int circle_len)
{
	...
	{
		return (val_a-val_b) - circle_len;
	}
	if(val_a-val_b < -circle_len/2)
	{
		return circle_len + (val_a-val_b);
	}
	return val_a - val_b;
}
	
	
static float pid_calculate(PID_TypeDef* pid, float measure, int circle_len)//, int16_t target)
{
  ...
  //	pid->target = target;
	...
	if(circle_len == 0)
	{
		...
		pid->err = circle_err(pid->target, pid->measure, circle_len);
		//pid->err = pid->target - pid->measure;
		//printf("measure:%d, target:%d, err:%d\n", (int)pid->measure, (int)pid->target, (int)pid->err);
	}
